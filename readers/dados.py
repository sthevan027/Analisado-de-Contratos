...
def _selecionar_aba(nomes_abas: list[str]) -> str:
    for nome in nomes_abas:
        if _normalizar(nome) == "DADOS":
            return nome
    raise ValueError("Nenhuma aba Dados encontrada na planilha")
...
def _localizar_cabecalho(linhas: list[tuple]) -> _Cabecalho:
    for indice, linha in enumerate(linhas):
        ...
        valor_unitario_col = next(
            (coluna for coluna, texto in normalizados.items() if "UNITARIO" in texto), None
        )
        valor_total_col = next(
            (coluna for coluna, texto in normalizados.items() if texto.endswith("TOTAL")), None
        )
        area_col = next(
            (coluna for coluna, texto in normalizados.items() if texto in ("AREA", "SECAO", "SEÇÃO".upper())),
            None,
        )
    ...
    raise ValueError("Cabeçalho da planilha Dados não foi encontrado nas primeiras 30 linhas")
...
def ler_dados(path: Path) -> ContratoNormalizado:
    ...
    grupo_atual = "Sem área informada"
    ...
    for linha in planilha.iter_rows(min_row=cabecalho.data_start, values_only=True):
        ...
        area_bruta = _celula(linha, cabecalho.area_col)
        area = str(area_bruta).strip() if area_bruta else grupo_atual
    ...
    return ContratoNormalizado(formato=FormatoContrato.DADOS, itens=tuple(itens))
